...
void setup() {
  ...
  rotationZ.attach(21);
  myservo.attach(12); // Use PWM pin 14 to control Sabertooth.
  myservo1.attach(13); // Use PWM pin 15 to control Sabertooth.
...
void loop() {
  ...
  if(moteurVisee == 1) {
   ...
   /*if (RC_VALUES[RC_CH2] > 1600){
    pastPosition = max(min(pastPosition - 1, 180), 0);
    rotationZ.write(pastPosition); 
    delay(10);
  }

  if (RC_VALUES[RC_CH2] < 1400){
    pastPosition = min(max(pastPosition + 1, 0), 180);
    rotationZ.write(pastPosition); 
    delay(10);
  }
  Serial.println(pastPosition);*/
  rotationZ.write(RC_VALUES[RC_CH2]);
