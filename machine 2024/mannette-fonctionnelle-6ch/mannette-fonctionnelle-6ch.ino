...
const int ENA_PIN_BOOM = 15; // the Arduino pin connected to the EN1 pin L298N
...
const int ENA_PIN_STICK = 4; // Changer port
const int IN1_PIN_STICK = 16; // Changer port
const int IN2_PIN_STICK = 17; //Changer port

const int ENB_PIN_BUCKET = 5; // Changer port
...
void setup() {
  ...
  pinMode(ENA_PIN_BOOM, OUTPUT);
  pinMode(IN1_PIN_BOOM, OUTPUT);
  pinMode(IN2_PIN_BOOM, OUTPUT);

  pinMode(ENB_PIN_BOOM, OUTPUT);
  pinMode(IN3_PIN_BOOM, OUTPUT);
  pinMode(IN4_PIN_BOOM, OUTPUT);

  pinMode(ENA_PIN_STICK, OUTPUT);
  pinMode(IN1_PIN_STICK, OUTPUT);
  pinMode(IN2_PIN_STICK, OUTPUT);

  pinMode(ENB_PIN_BUCKET, OUTPUT);
  pinMode(IN3_PIN_BUCKET, OUTPUT);
  pinMode(IN4_PIN_BUCKET, OUTPUT);

  digitalWrite(ENA_PIN_BOOM, HIGH);
...
void loop() {
  ...
  if (moteurVisee == 0){
     if (RC_VALUES[RC_CH2] > 1600){
      ...
      digitalWrite(ENA_PIN_BOOM, LOW);
      
    }
   }

   else if (moteurVisee == 1){
     if (RC_VALUES[RC_CH2] > 1600){
         digitalWrite(ENA_PIN_STICK, HIGH);
         ...
         delay(10);
          ...
          digitalWrite(ENA_PIN_STICK, LOW);
      }
   }

   else {
      if (RC_VALUES[RC_CH2] > 1600){
         digitalWrite(ENB_PIN_BUCKET, HIGH);
         ...
         delay(10);
          ...
          digitalWrite(ENB_PIN_BUCKET, LOW);
      }
   }
